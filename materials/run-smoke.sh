#!/bin/bash
# materials/run-smoke.sh — drive the master-spec smoke from .cpanel.yml.
# Same shape as tasks/run-smoke.sh: HTTP loopback first, CLI fallback.
set +e

HERE="$(cd "$(dirname "$0")/.." && pwd)"
TARGET="http://127.0.0.1/materials/smoke_internal.php"
...
cd "$HERE" && php materials/smoke_internal.php
