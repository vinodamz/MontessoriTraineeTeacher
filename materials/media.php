 ...
 *
 * Access, either:
 *   - a logged-in user with the materials module, or assigned the daily check,
 ...
 *
 *   GET ?id=N                 monthly (mm_condition_media)
 *   GET ?id=N&daily=1         daily walk (mm_daily_media)
 *   GET ?id=N&thumb=1         480px JPEG thumbnail
 */
...
if ($daily) {
    $st = db()->prepare("
        SELECT md.stored_filename, md.mime_type, md.original_filename, md.kind
    ...
    $u = current_user();
    if ($u === null || !mm_can_daily_check($u)) { http_response_code(404); exit('Not found.'); }
} else {
    $st = db()->prepare("
