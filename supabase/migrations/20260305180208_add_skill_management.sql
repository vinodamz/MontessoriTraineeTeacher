/*
  # Skill Indicator Management

  ## Overview
  Adds the ability to configure skill indicators per grade/category via admin UI,
  and allows teachers to add custom per-student indicators.

  ## Changes

  ### skill_indicators table
  - Add `is_active` boolean column (default true) to allow disabling without deleting

  ### New Table: student_custom_indicators
  - Stores additional/custom skill indicators added for a specific student
  - Teachers can add these from the assessment modal
  - Has category, indicator text, display order, active flag
  - Linked to student and teacher who created it

  ## Security
  - RLS enabled on new table
  - Anyone can read/write (consistent with app pattern)
*/
